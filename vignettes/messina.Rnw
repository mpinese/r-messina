...
expression analysis is described in the Messina paper
\texttt{r citep("10.1371/journal.pone.0005337")}.

\section{Using Messina to find genes that optimally distinguish 
...
leukemia expression dataset in Bioconductor package leukemiasEset:
...
conditions, so filtering is not necessary for FDR control
\texttt{r citep("10.1371/journal.pone.0005337")}.
...
\section{Using Messina to detect differential expression in the
...
\section{Using Messina to determine the best features to predict
survival outcome}
