...
#'   \describe{
#'     \item{"coxcoef-exact"}{The coefficient of a Cox proportional hazards fit to the model Surv ~ I(x > T),
...
#'     \item{"coxcoef-approx"}{A fast approximation of "coxcoef-exact".}
...
#'   "tau", "reltau", "coxcoef-approx", or "coxcoef-exact"; see details for more information.
...
messinaSurv = function(x, y, obj_min, obj_func, min_group_frac = 0.1, f_train = 0.8, n_boot = 50, seed = NULL, parallel = NULL, silent = FALSE)
{
	...
	temp = boot_objs
	temp[is.na(temp)] = c("tau" = 0.5, "reltau" = 0.5, "coxcoef-exact" = 0, "coxcoef-approx" = 0)[obj_func]
...
messinaSurvObjectiveFunc = function(x, y, func)
{
	...
	else if (func == "coxcoef-approx")
	{
		# Use Messina-specific approximate Cox fit implementation
		n <- length(x)
		time = y[,1]
		status = y[,2]
		sorted <- order(time)
		...
		sstat <- as.integer(status[sorted])
		sx <- as.logical(x[sorted])
		lrt_stat = messinaSurvLRT(sx, stime, sstat)
		return(lrt_stat * sqrt(4/sum(status)))
	}
	else
	{
